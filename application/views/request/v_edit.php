    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="card-tools">
          <a href="<?php echo base_url() . 'c_request/index' ?>" class="btn btn-secondary"><i class="fas fa-arrow-alt-circle-left"></i> Kembali</a>
        </div>
      </div>
      <div class="card-body">
        <div class="row">
            ...
            <form class="form-horizontal" id="form">
              <div class="card-body">
                <div class="form-group row">
                  <label for="formKode" class="col-sm-4 control-label">Kode Request</label>
                  <div class="col-sm-8">
                    <input type="text" name="KodeRequest" class="form-control" id="formKode" readonly="readonly" value="<?php echo $req->Kode_Request ?>">
                  </div>
                </div>
                <div class="form-group row">
...
<script>
  $(function() {
    let idRequest = "<?php echo $req->Kode_Request ?>";
    let total;
    ...
    $('#formAdd').on('submit', function(e) {
      e.preventDefault();
      $.ajax({
        url: "<?php echo base_url() . 'c_request/addBarang' ?>",
        method: "POST",
        dataType: "JSON",
        contentType: false,
        processData: false,
    ...
    $('#btnSave').on('click', function() {
      $.ajax({
        url: "<?php echo base_url() . 'c_request/updateRequest/' ?>" + idRequest,
        method: "POST",
        dataType: "JSON",
        data: {
          Kode: $('[name=KodeRequest]').val(),
          Total: total,
        ...
        success: function(respon) {
          ...
          if (respon.success) {
            Alert.fire({
              type: 'success',
              title: 'Request barang berhasil.'
            }).then((result) => {
              window.location = "<?php echo base_url() . 'c_request/index' ?>";
            });
          }
        }

...
<script>
  var dataBarang = JSON.parse('<?php echo isset($detilBarangSup) ? $detilBarangSup : "{}" ?>');
  for (let x = 0; x < dataBarang.length; x++) {
    $('input[name^="listBarang"]').each(function() {
      if ($(this).val() == dataBarang[x].Kode_Barang) {
        $(this).iCheck('check');
      }
    });
  }
